...
| **过程** | Autorefine [84]       | 检索-搜索交互 | 检索特定奖励     | $r = I[a_{gt} \cap a_{refine} = a_{gt}], \text{其中} a*{refine} = \bigcup*{t} \{c_t                                                                                       | (s_t, c_t) \in a, s_t = \langle ref \text{ine} \rangle\}$ |
|          | R-Search [244]        | 检索-搜索交互 | 证据质量奖励     | $r = F1(\alpha*{cf}, \alpha*{gold}), \alpha*{cf} \sim \pi*{cf}(\cdot                                                                                                      | q, e)$                                                    |
...
| ------------------------- | ---------------------- | -------- | -------- | ------------------- | -------- | ------------------------------ | --------------------------------- | --------------- | ------------------------------------------- | --------------------------- |
| Search-R1 [84]            | 自适应搜索             | ✗        | 真实世界 | PPO GRPO            | 基于规则 | ORM                            | 答案 EM                           | 单代理          | [68, 86, 90, 127, 144, 191, 231]            |
| ReSearch [26]             | 自适应搜索             | ✗        | 真实世界 | GRPO                | 基于规则 | ORM                            | 格式<br>答案 F1                   | 单代理          | [68, 144, 191, 231]                         |
| AutoCoA [242]             | 自适应搜索             | ✓        | 真实世界 | GRPO                | 基于规则 | ORM                            | 格式<br>答案 EM                   | 单代理          | [68, 86, 90, 127, 144, 191, 231]            |
| SimpleDeep-Searcher [177] | 自适应搜索             | ✓        | 真实世界 | DPO<br>Reinforce++  | 基于规则 | ORM                            | 格式<br>答案 F1                   | 单代理          | [68, 89, 132, 144, 184, 191, 205, 231, 250] |
| ExSearch [167]            | 自适应搜索             | ✗        | 真实世界 | GEM                 | PRM      | 轨迹质量                       | 单代理                            | [90, 191, 231]  |
| IKEA [73]                 | 搜索效率               | ✗        | 真实世界 | GRPO                | 基于规则 | ORM                            | 格式<br>答案 EM<br>知识边界       | 步级            | [68, 90, 127, 231]                          |
